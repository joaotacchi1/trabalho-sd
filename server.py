...
class NTPServer:
    ...
    def get_ntp_time(self):
        # Obtém a hora real de um servidor NTP
        try:
    ...
    def run(self):
        ...
        while True:
            ...
            if data.decode() == "REQUEST_TIME":
                ...
                send_time = current_time + (network_delay / 2)
                self.sock.sendto(str(send_time).encode(), addr)
                print(f"Enviado tempo {datetime.fromtimestamp(send_time)} para {addr} com delay {network_delay:.3f}s")

if __name__ == "__main__":
    server = NTPServer(host="192.168.1.10")
    server.run()